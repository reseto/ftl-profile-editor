...
public class SavedGameSectorMapPanel extends JPanel {
	...
	private void showLayoutEditor() {
			...
			public void actionPerformed( ActionEvent e ) {
				...
				if ( LAYOUT_GRID.equals( editorPanel.getCombo( LAYOUT ).getSelectedItem() ) ) {
					...
					if ( sectorLayoutSeed == newSeed && newGenMap.getGeneratedBeaconList().size() != beaconRefs.size() ) {
						...
						String badCountBuf = "The RNG-informed map has a beacon count that doesn't match the fixed beacon " +
								"list. FTL must have used a different RNG.";

						JOptionPane.showMessageDialog( frame, badCountBuf, "Bad RNG", JOptionPane.WARNING_MESSAGE );
						newGenMap = null;
					}
				}
		...
		String noticeBuf = "A sector map is part random, part fixed & editable.\n" +
				"\n" +
